...
bool SWRLReasoner::loadSWRLFile(const DataSourcePtr &dataFile) {
	...
	return PROLOG_ENGINE_EVAL(getReasonerQuery(PrologTerm(consult_f, path.native())));
}

bool SWRLReasoner::initializeDefaultPackages() {
	KB_DEBUG("Initializing SWRL module.");
	return consult(std::filesystem::path("reasoner") / "swrl" / "__init__.pl", nullptr, false);
}
